...
export function Banner({ version, port, stages, llmLinguaUrl, outputMode, level }) {
  ...
  const stagesHeader = hasLevel
    ? h(Text, { bold: true }, `Stages `, h(Text, { dimColor: true }, `(${stages.length} active \u2192 expand with TAMP_STAGES)`))
  ...
  return h(Box, { flexDirection: 'column', paddingX: 1 },
    ...
    hasLevel ? h(Text, null, '') : null,
    hasLevel ? h(Text, null,
      h(Text, { bold: true }, 'Level '),
      h(Text, { color: 'yellow' }, String(level)),
      h(Text, { dimColor: true }, ` (${stages.length} stages \u2014 tamp settings to change)`),
    ) : null,
    h(Text, null, ''),
    ...
    stagesHeader,
    hasLevel ? null : h(Box, { paddingLeft: 2, flexDirection: 'column' },
      ...defaultActive.map(s =>
        h(Text, { key: s },
          h(Text, { color: 'green' }, '\u2713 '),
          h(Text, { color: 'cyan' }, s),
    ...
    !hasLevel && disabled.length ? h(Text, null, '') : null,
    !hasLevel && disabled.length ? h(Text, { bold: true }, 'Available ', h(Text, { dimColor: true }, '(not active — opt in for more savings)')) : null,
    !hasLevel && disabled.length ? h(Box, { paddingLeft: 2, flexDirection: 'column' },
      ...disabled.flatMap(s => {
