...
// Per-file dot-progress: "<path>.py[::node] <status run> [ NN% ]" (percent
// optional). Anchored to a .py path prefix so a traceback/assertion line that
// merely ends in "..." is NOT mistaken for progress and silently dropped.
const DOT_PROGRESS = /^\S*\.py(::\S+)?\s+[.FsxE]{2,}\s*(\[\s*\d+%\s*\])?\s*$/;
// Wrapped progress continuation: a bare status run followed by a percent.
const PCT_PROGRESS = /^\s*[.FsxE]{2,}\s*\[\s*\d+%\s*\]\s*$/;
...
export default {
  ...
  rewrite(text) {
    ...
    for (const line of lines) {
      ...
      // Drop per-file / wrapped dot-progress lines. Both patterns are anchored
      // (path prefix or trailing percent), so traceback/assertion lines that end
      // in "..." are preserved without needing a fragile keyword guard.
      if (DOT_PROGRESS.test(line) || PCT_PROGRESS.test(line)) continue;
      out.push(line);
    }
