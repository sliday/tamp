...
export default {
  ...
  rewrite(text) {
    ...
    for (const line of lines) {
      ...
      // BuildKit "in-progress" step frames carry a [x/y] step counter and end
      // with a fractional-second marker; they are re-emitted as DONE later, so
      // strip the non-DONE variants. Requiring [x/y] avoids eating a RUN step's
      // own stdout (prefixed "#N <timestamp> ...", no counter), which can
      // legitimately end in a duration (e.g. "webpack compiled in 2.5s"). The
      // error/warning/fail guard stays as belt-and-suspenders.
      if (/^#\d+\s+\[\d+\/\d+\]\s.*\s\d+\.\d+s$/.test(line) && !/\sDONE\b/.test(line) && !/\b(error|warning|fail)/i.test(line)) continue;
      out.push(line);
    }
