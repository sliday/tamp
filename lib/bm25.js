...
export function trimLinesByRelevance(text, query, {
  ...
  function closureFor(i) {
    ...
    if (ownClosers) {
      for (const c of ownClosers) {
        if (c < N && c !== i && !keep[c] && !pending.includes(c)) {
          pending.push(c)
          cost += lineTokens[c]
        }
      }
    ...
    // IS a closer. Walk the outermost of them; the inner ones are its
    ...
    if (ownHeaders && ownHeaders.length > 0) {
      for (const h of ownHeaders) {
        if (!keep[h] && !pending.includes(h)) {
          pending.push(h)
          cost += lineTokens[h]
        }
      }
      p = Math.min(...ownHeaders)
