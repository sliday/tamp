...
// Keyword alternations need \b on both ends: unanchored, `latest`/`contest` hit
// `test` and `specific`/`inspect` hit `spec`, and one false positive disables
// disclosure (compress.js:136) and bm25-trim (compress.js:168) for the session.
const DANGEROUS_TASK_PATTERNS = [
  /\bsecurity\b|\bvulnerabilit\w*|\bexploit\w*|\battack\w*/i,
  /\bdebug\w*|\binvestigat\w*|\bdiagnos\w*|\btroubleshoot\w*/i,
  /\bmemory leak\b|\bperforman\w*|\boptimi[sz]\w*/i,
  /^refactor|^architecture|^design/i,
  /^fix\s+bug/i,
  /^explain|^why|^how\s+(does|work)/i,
  /\btest(s|ing)?\b|\bspecs?\b|\bcoverage\b/i,
]
...
/**
 * True if the user's latest message matches a DANGEROUS_TASK_PATTERNS entry.
 * Used by lossy stages (e.g., disclosure) to bypass when the task demands
 * full fidelity (security/debug/refactor/test).
 */
export function isDangerousTask(userMessage) {
  return detectTaskType(userMessage) === 'dangerous'
}
