...
function generateAbstract(analysis) {
  ...
  return `We present a controlled A/B benchmark measuring Tamp's token compression effectiveness across seven representative agentic coding scenarios. Using OpenRouter as an independent measurement oracle with Claude Sonnet 4, we find a weighted average reduction of ${pct(agg.weighted_pct_reduction)} in input tokens. The strongest compression occurs on tabular/array data (${pct(best.savings.pct.mean)} for ${best.name}), while source code and error results correctly pass through unmodified. Across all compressible scenarios, output tokens remain identical between control and treatment, confirming semantic preservation. At current pricing ($3.00/Mtok input), Tamp saves approximately $${agg.session_projection.dollars_saved_per_session.toFixed(2)} per 200-request coding session with sub-millisecond compression overhead.`
}
